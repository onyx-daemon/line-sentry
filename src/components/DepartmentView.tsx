...
import { 
  ...
  updateMachineStatus,
  fetchMachineStats,
  clearError as clearMachineError
...
const DepartmentView: React.FC = () => {
  ...
  // Redux state
  const { currentDepartment: department, loading: departmentLoading } = useAppSelector((state) => state.departments);
  const { machines, machineStats, machineStatuses, loading: machineLoading } = useAppSelector((state) => state.machines);
  
  const loading = departmentLoading || machineLoading;
  ...
  // Update positions and dimensions when machines change
  useEffect(() => {
